...
export function LaunchToolTile({ tool }: LaunchToolTileProps) {
	...
	return (
		...
		>
			<div
				className={`h-full rounded-[2rem] border p-1.5 ${isPrimary ? "bg-white" : "bg-gray-0"}`}
			>
				<Stack
					gap="lg"
					className="h-full rounded-[calc(2rem-0.375rem)] border p-7"
					justify="space-between"
				>
					<Group justify="space-between" align="flex-start">
						<ThemeIcon color={tool.color} variant="light" size={isPrimary ? 52 : 44}>
							<Icon size={isPrimary ? 24 : 20} strokeWidth={1.5} />
						</ThemeIcon>
						<span className="flex h-9 w-9 items-center justify-center rounded-full bg-gray-1">
