...
function UploadDocumentsOverviewInner({ scripts, onAddedToInvoice }: UploadDocumentsOverviewProps) {
	...
	const allSelected = scripts.length > 0 && selectedScriptIds.size === scripts.length;
	...
	const handleAddToInvoice = useCallback(() => {
		...
		if (addMode === "existing" && selectedItemId) {
			...
			if (!itemExists) {
				setAddError("The selected invoice item was removed. Please choose another item or create a new one.");
				return;
			}
		}
		setAddError(null);
		...
		if (addMode === "existing" && selectedItemId) {
			addSubitemsToItem(selectedIds, selectedItemId, selectedScripts, subitemLabel.trim(), rate);
		} else if (addMode === "new" && newItemName.trim()) {
			addSubitemsAsNewItem(selectedIds, newItemName.trim(), selectedScripts, subitemLabel.trim(), rate);
		} else return;
	...
	return (
		...
		>
			<Stack gap="md">
				...
				{selectedPreset ? (
					<Text size="sm" c="dimmed">
						Rate: {selectedPreset.rateAmount} per {selectedPreset.ratePerWords} words
					</Text>
				) : (
					<>
						<Box>
